...
我们的 Python 代码不是独立服务，而是 Hermes-Agent runtime 加载的 **plugin**。集成面必须先于业务代码定型，否则所有 hook / skill / tool 都是"漂"的：

| 集成点 | 形式 | 我们的代码位置 |
| --- | --- | --- |
| **Webhook routes** | `config/hermes/routes.yaml`（Hermes 加载） | YAML 配置 + payload 模板 |
| **Event hooks** | `@hook("gateway:webhook_received")` 等装饰器（来自 `hermes_agent.plugin`） | `src/aiops/hermes_plugin/hooks.py` |
| **Tools (plugin)** | `@tool` 装饰器 + 注册到 `pyproject.toml` entry point `hermes_agent.plugins` | `src/aiops/plugins/<intent>.py` |
| **Skills (SOP)** | Markdown 文件 + frontmatter，放 `wiki/` 下并在 Hermes config 指向 | `wiki/<分类>/*.md` |
| **Bot commands** | 带 `command:` frontmatter 的 skill，Hermes 自动注册为 `/<name>` 命令 | `src/aiops/bot/skills/*.md` 或独立目录 |
| **Context Files** | 启动时通过 `HERMES_CONTEXT_FILES` 或 config 加载 | wiki `_index.md` 文件 |
| **MCP servers** | Hermes config 注册外部 MCP endpoint | NetBox MCP / 自建 MCP |

**Hermes plugin entry point 模式**：
...
from hermes_agent.plugin import hook, tool, register_command  # 来自 Hermes SDK

def register_hooks() -> None:
    from . import hooks                     # 让 Hermes 发现并装载
    return [hooks.dedupe_and_persist, hooks.cost_cap, hooks.kill_switch_check, ...]

def register_tools() -> None:
    from aiops.plugins import scrapli_tools, ansible_tools
    return [scrapli_tools.get_interface_status, ...]
```

**部署前置验证（plan Phase 1 必须先跑通）**：
- 装一个 Hermes 实例 + 加载我们的 plugin entry point
- 通过 `hermes plugins list` 看到我们的 hooks / tools / commands
- 发一个 fake webhook，验证 `gateway:webhook_received` hook 被触发
- 在飞书 Bot 里输入 `/aiops ping`，验证 bot command 路由通
...
```python
# plugins/aiops_alert_processor.py
@hook("gateway:webhook_received")
async def dedupe_and_persist(payload, route_name):
    # 1. 幂等：Zabbix event_id 作为唯一键
    event_id = payload["event"]["eventid"]
    if await db.alert_exists(event_id):
        return RETURN_CACHED(event_id)

    # 2. 兜底去重：同 host + trigger 5 分钟窗口
    key = f"{payload['host']['host']}:{payload['trigger']['name']}"
    if await redis.set(key, "1", ex=300, nx=True) is None:
        return SKIP

    # 3. 入库 + 风险分级（查 NetBox criticality + Fast Path 规则）
    await db.insert_alert(payload, route_name, event_id=event_id)
    device = await netbox.get_device(payload["host"]["host"])
...
@hook("pre_tool_call")
async def early_validate(tool_name, args, context):
...
```python
@hook("pre_llm_call")
async def sanitize_untrusted_payload(messages, context):
